...
const OTPScreen = () => {
  ...
  // 2. Retrieve params using the hook (NOT route.params)
  ...
  const handleVerify = async () => {
    ...
    try {
      console.log('Verifying OTP at:', API_URLS.VERIFY_OTP);
      const payload = { 
        email:email,
      ...
      if (response.ok) {
        ...
        if (type === 'RESET_PASSWORD') {
          // If this was for password reset, go to reset password screen
          router.push({
            pathname: '/ResetPasswordScreen',
            params: { email, otp } // Pass OTP as proof if needed
          });
        } else {
